...
import { Plus, Receipt, ChevronDown, ChevronRight, User, AlertCircle } from 'lucide-react';
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="bg-gradient-to-br from-rose-500 to-rose-600 rounded-2xl p-4 text-white shadow-lg shadow-rose-500/20">
                <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={() => setShowAddModal(true)}
                        className="w-9 h-9 bg-white/20 hover:bg-white/30 rounded-xl flex items-center justify-center transition-all active:scale-90 backdrop-blur-sm">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white/15 backdrop-blur-sm rounded-xl p-3">
                        ...
                        {totalBs && <p className="text-[11px] text-white/50 font-bold mt-0.5">Bs {totalBs}</p>}
                    </div>
                    <div className="bg-white/15 backdrop-blur-sm rounded-xl p-3">
            ...
            <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 rounded-xl p-1">
                {[
                    { id: 'pending', label: 'Pendientes', count: debts.filter(d => d.status === 'pending').length },
                    { id: 'paid', label: 'Pagadas', count: debts.filter(d => d.status === 'paid').length },
                    { id: 'all', label: 'Todas', count: debts.length },
                ].map(f => (
                    <button key={f.id} onClick={() => setFilter(f.id)}
                        className={`flex-1 py-2 text-[11px] font-bold rounded-lg transition-all flex items-center justify-center gap-1 ${filter === f.id
                            ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm'
                            : 'text-slate-400 hover:text-slate-600'
                        }`}>
                        {f.label}
                        <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${filter === f.id ? 'bg-rose-100 text-rose-600' : 'bg-slate-200 dark:bg-slate-600 text-slate-400'}`}>
                            {f.count}
                        </span>
                    </button>
                ))}
                    ...
                    {staffList.map(staff => {
                        ...
                        return (
                            <div key={staff.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-left">
                                            ...
                                            <p className="text-[10px] text-slate-400 font-medium">
                                                {pendingDebts} {pendingDebts === 1 ? 'deuda' : 'deudas'} pendiente{pendingDebts !== 1 ? 's' : ''}
                                    ...
                                    <div className="flex items-center gap-2">
                                        {staff.totalPending > 0 && (
                                            <div className="text-right">
                                                <span className="bg-rose-500 text-white text-[10px] font-black px-2.5 py-1 rounded-full block">
                                                    ${staff.totalPending.toFixed(2)}
                                                </span>
                                                {bsTotal && (
                                                    <p className="text-[9px] text-slate-400 font-bold mt-0.5">Bs {bsTotal}</p>
                                                )}
                                            </div>
                                        )}
                                ...
                                {isOpen && (
                                    <div className="border-t border-slate-100 dark:border-slate-700 px-3 py-2 space-y-1.5 bg-slate-50/50 dark:bg-slate-800/30">
                                        {staff.debts.map(d => {
                                            ...
                                            return (
                                                <button key={d.id} onClick={() => setSelectedDebt(d)}
                                                    ...
                                                    <div className="text-right ml-3 shrink-0">
                                                        <span className={`text-sm font-black ${d.status === 'paid' ? 'text-emerald-500' : 'text-rose-600'}`}>
